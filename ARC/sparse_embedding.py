...
class CastedSparseEmbedding(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        selected_weights = self.weights[inputs]
        
        with torch.no_grad():
            # Handle dynamic batch sizes by resizing if necessary
            if self.local_weights.shape[0] != actual_batch_size:
                # Create new tensors with correct batch size
                device = self.local_weights.device
                embedding_dim = self.local_weights.shape[1]
                
                # Update local_weights size
                self.local_weights.data = torch.zeros(actual_batch_size, embedding_dim, 
                                                    device=device, dtype=self.local_weights.dtype,
                                                    requires_grad=True)
                
                # Update local_ids size  
                self.local_ids.data = torch.zeros(actual_batch_size, dtype=torch.int32, device=device)
            
            self.local_weights.data.copy_(selected_weights)
