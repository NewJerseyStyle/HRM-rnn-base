...
@torch.jit.unused
def elementwise_recurrence_naive(
    U: Tensor,
    x: Tensor,
    weight_c: Tensor,
    bias: Tensor,
...
# --- End of elementwise_recurrence_naive ---

# --- Start of SRUCell from sru/modules.py ---
class SRUCell(nn.Module):
    ...
    __constants__ = ['input_size', 'hidden_size', 'output_size', 'rnn_dropout',
                     ...
                     'v1', 'rescale', 'activation_type', 'activation',
                     'transform_module', 'projection_size', 'num_matrices',
                     'layer_norm', 'scale_x', 'normalize_after', 'weight_c_init', ]

    scale_x: Tensor

    def __init__(self, input_size: int, hidden_size: int, dropout: float = 0.0,
                 rnn_dropout: float = 0.0, bidirectional: bool = False,
                 projection_size: int = 0, highway_bias: float = 0.0,
                 layer_norm: bool = False, normalize_after: bool = True,
                 transform_module: Optional[nn.Module] = None,
                 rescale: bool = False, has_skip_term: bool = True,
                 use_tanh: bool = False, v1: bool = False,
        ...
        rnn_dropout: float, optional
            ...
            This option is deprecated because minimal performance improvement,
            and increases codebase size. This option will be removed at the
            next major version upgrade
        bidirectional: bool, optional
            if True, set the module as a bidirectional SRU (default=False)
        projection_size: int, optional
            if non-zero, factorize the ``weight`` parameter matrix as a product
            of two parameter matrices, using an innder dimension
            ``projection_size`` (default=0)
        use_tanh: bool, optional
            [DEPRECATED] if True, apply `tanh` activation to the hidden state
            (default=False). `tanh` is deprecated because minimal performance
            improvement, and increases codebase size. This option will be
            removed at the next major version upgrade.
        highway_bias: float, optional
            the initial value of the bias used in the highway (sigmoid) gate
            (defulat=0)
        has_skip_term: bool, optional
            whether to include a residual connection for output hidden state `h`
            (default=True)
        layer_norm: bool, optional
            whether to apply pre- layer normalization for this layer
            (default=False)
        rescale: bool, optional
            whether to apply a constant rescaling multiplier for the residual
            term (default=True)
        v1: bool, optional
            [DEPRECATED] whether to use the an ealier v1 implementation of SRU
            (default=False)
        transform_module: nn.Module, optional
            use the give module instead of the batched matrix multiplication to
            compute the intermediate representations U needed for the
            elementwise recurrrence operation (default=None)
        amp_recurrence_fp16: Type, optional
            When using AMP autocast, selects which type to use for recurrence
            custom kernel. False: torch.float32, True: torch.float16
        normalize_after: bool, optional
            if True use post layer norm, else pre layer norm (default=True)
        weight_c_init: float, optional
            size of uniform initiatialization of weight_c (default=1.0)
        """
    ...
    def forward(self, input: Tensor, c0: Optional[Tensor] = None,
        ...
        h, c = self.apply_recurrence(U, V, residual, c0, scale_val, mask_c,
                                     mask_pad)
    ...
    def apply_recurrence(self, U: Tensor, V: Tensor, residual: Tensor, c0: Tensor,
                         scale_val: Optional[Tensor], mask_c: Optional[Tensor],
    ...
    def compute_UV(self, input: Tensor, c0: Optional[Tensor],
                   mask_pad: Optional[Tensor]) -> Tuple[Tensor, Tensor]:
        """ SRU performs grouped matrix multiplication to transform the input
        (length, batch_size, input_size) into a tensor U of size
        (length * batch_size, output_size * num_matrices). U will be computed
        by the given transform_module. The module can optionally return an
        additional tensor V (length, batch_size, output_size * 2) that will be
        added to the hidden-to-hidden coefficient terms in sigmoid gates, i.e.,
...
# --- Start of SRU from sru/modules.py ---
class SRU(nn.Module):
    ...
    __constants__ = ['input_size', 'hidden_size', 'output_size', 'num_layers',
                     ...
                     'num_directions', 'nn_rnn_compatible_return',
                     'input_to_hidden']

    def __init__(self, input_size: int, hidden_size: int, num_layers: int = 2,
                 dropout: float = 0.0, rnn_dropout: float = 0.0,
                 bidirectional: bool = False,
                 projection_size: Union[int, Sequence[int]] = 0,
                 highway_bias: float = 0.0, layer_norm: bool = False,
                 normalize_after: bool = False,
                 transform_module: Optional[Union[nn.Module, Sequence[nn.Module]]] = None,
                 has_skip_term: bool = True, rescale: bool = False,
                 use_tanh: bool = False, v1: bool = False,
        ...
        projection_size: Union[int, Sequence[int]]
            if non-zero, factorize the ``weight`` parameter in each layeras a
            product of two parameter matrices, using an inner dimension
            ``projection_size`` (default=0)
            If a sequence, length must equal number of layers, and values are
            projection size for each layer
        use_tanh: bool, optional
            [DEPRECATED] if True, apply `tanh` activation to the hidden state
            (default=False). `tanh` is deprecated because minimal performance
            improvement, and increases codebase size. This option will be
        ...
        transform_module: Union[nn.Module, Sequence[nn.Module]], optional
            ...
            multiplication to compute the intermediate representations U needed
            for the elementwise recurrrence operation. The module must take
            input x of shape (seq_len, batch_size, hidden_size). It returns a
            tensor U of shape (seq_len, batch_size, hidden_size * num_matrices),
            and one optional tensor V of shape (seq_len, batch_size,
            hidden_size * 2). (default=None)
        amp_recurrence_fp16: Type, optional
            When using AMP autocast, selects which type to use for recurrence
            custom kernel. False: torch.float32, True: torch.float16
        ...
        for i in range(num_layers):
            ...
            if transform_module is not None:
                transform_module_i = transform_module[i] \
                    if isinstance(transform_module, list) else \
                    copy.deepcopy(transform_module)

            _projection_size = projection_size \
                if isinstance(projection_size, int) else \
                projection_size[i]
    ...
    def forward(self, input: Tensor, c0: Optional[Tensor] = None,
        ...
        c0: Tensor, optional
            the initial internal hidden state. shape: (num_layers, batch_size,
            output_size) where output_size = hidden_size * num_direction
        mask_pad: Tensor, optional
            the mask where a non-zero value indicates if an input token is pad
            token that should be ignored in forward and backward computation.
            shape: (length, batch_size)

        Returns
        ----------
        h: Tensor
            the output hidden state. shape: (length, batch_size, output_size)
            where output_size = hidden_size * num_direction
        c: Tensor
            the last internal hidden state. shape: (num_layers, batch_size,
            output_size), or (num_layers * num_directions, batch_size,
            hidden_size) if `nn_rnn_compatible_return` is set `True`
        """
        ...
        if c0 is None:
            zeros = torch.zeros(input.size(1), self.output_size,
                                dtype=input.dtype, device=input.device)
...
# --- Start of SRUppProjectedLinear from sru/modules.py ---
class SRUppProjectedLinear(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, proj_features: int,
        ...
        proj_features: int
            the number of features used for attention computation. The input is
            projected into this dimension first. After that the module apply the
            query-key-value attention computation. The output is projected to
            dimension `out_features`.
        dropout: float, optional
            dropout probability applied after attention computation and before
            the final projection (default=0.0).
        layer_norm: bool, optional
            whether to apply layer normalization within the projected linear
            module.
        """
    ...
    def forward(self, input: Tensor, mask_pad: Optional[Tensor] = None,
                attn_mask: Optional[Tensor] = None, memory: Optional[Tensor] = None,
...
# --- Start of SRUppAttention from sru/modules.py ---
class SRUppAttention(nn.Module):
    """ Self-attention module used in SRU++ module. """

    __constants__ = ['in_features', 'out_features', 'proj_features',
                     'num_heads', 'attn_dropout', 'rezero_init_alpha']

    def __init__(self, in_features: int, out_features: int, proj_features: int,
                 num_heads: int = 1, dropout: float = 0.0,
                 attn_dropout: float = 0.0, rezero_init_alpha: float = 0.0,
                 layer_norm: bool = False, normalize_after: bool = True):
        ...
        num_heads: int, optional
            the number of attention heads used. `proj_features` must be
            multipler of this value (default=1).
        dropout: float, optional
            dropout probability applied after attention computation and before
            the final projection (default=0.0).
        ...
        rezero_init_alpha: float, optional
            initial scalar value for the attention transformation
            `x + alpha * Attention(x)` (default=0).
        normalize_after: bool, optional
            if True, apply post layer normalization; otherwise apply pre layer
    ...
    def forward(self, input: Tensor, mask_pad: Optional[Tensor] = None,
        ...
        if attn_mask is not None:
            if list(attn_mask.size()) != [tgt_len, src_len]:
                raise ValueError("attn_mask has size {} but expect {}.".format(
                    list(attn_mask.size()), [tgt_len, src_len]
                ))
        ...
        if mask_pad is not None or memory_mask_pad is not None:
            ...
            if list(mask_pad.size()) != [src_len, bsz]:
                raise ValueError("mask_pad has size {} but expect {}.".format(
                    list(mask_pad.size()), [src_len, bsz]
                ))
        ...
        return attn_output
# --- End of SRUppAttention ---

# --- Start of SRUppCell from sru/modules.py ---
class SRUppCell(SRUCell):
    """ A single layer of SRU++, inherited from SRUCell module """

    def forward(self, input: Tensor, c0: Optional[Tensor] = None,
                mask_pad: Optional[Tensor] = None, attn_mask: Optional[Tensor] = None,
        ...
        U = transform_module(input, mask_pad=mask_pad, attn_mask=attn_mask,
                             memory=memory, memory_mask_pad=memory_mask_pad)
        V = self.weight_c
...
# --- Start of SRUpp from sru/modules.py ---
class SRUpp(nn.Module):
    ...
    __constants__ = ['input_size', 'hidden_size', 'proj_size', 'output_size',
                     ...
                     'use_layer_norm', 'num_directions',
                     'nn_rnn_compatible_return', 'input_to_hidden', 'rnn_lst',
                     'normalization_type']

    def __init__(self, input_size: int, hidden_size: int, proj_size: int,
                 num_layers: int = 2, dropout: float = 0.0,
                 attn_dropout: float = 0.0, num_heads: int = 1,
                 bidirectional: bool = False, layer_norm: bool = False,
                 normalize_after: bool = False, attn_layer_norm: bool = True,
                 highway_bias: float = -2.0, attention_every_n_layers: int = 1,
                 attention_last_n_layers: int = -1, rescale: bool = False,
                 nn_rnn_compatible_return: bool = False,
                 proj_input_to_hidden_first: bool = False,
                 weight_c_init: float = 1.0):
        """Initialize the SRU++ module.
        ...
        layer_norm: bool, optional
            whether to apply layer normalization to each SRU++ layer
            (default=False).
        normalize_after: bool, optional
            whether to apply post layer norm that normalizes the output of each
            SRU++ layer (default=False).
        attn_layer_norm: bool, optional
            whether to apply layer norm in the attention module or projected
            linear module if attention is disabled (default=True).
        highway_bias: float, optional
            the initial value of the bias used in the highway (sigmoid) gate
            (default=-1.0).
        attention_every_n_layers: int, optional
            only introduce attention every few layers of SRU++. by default,
            every SRU++ layer has attention (default=1).
        rescale: bool, optional
            whether to apply a constant rescaling multiplier for the residual
            term (default=False).
        proj_input_to_hidden_first: bool, optional
            if True, apply an nn.Linear module to the input of this module when
            input_size != hidden_size (default=False).
        weight_c_init: float, optional
            size of uniform initiatialization of weight_c (default=1.0)
        """
        ...
        for i in range(num_layers):
            ...
            if use_attention(i):
                custom_m = SRUppAttention(
                    in_features, out_features, proj_features,
                    dropout=dropout, attn_dropout=attn_dropout,
                    num_heads=num_heads, layer_norm=attn_layer_norm,
                )
            else:
                custom_m = SRUppProjectedLinear(
                    in_features, out_features, proj_features,
                    dropout=dropout, layer_norm=attn_layer_norm,
                )
    ...
    def forward(self, input: Tensor, c0: Optional[Tensor] = None,
        ...
        attn_mask: Tensor, optional
            ...
            attention weights before softmax normalization. input_length is the
            length of the input tensor, and context length is the total length
            of context states that each input can attend to. the context_length
            is equal to the sum of input_length and the lengths of extra memory
            states given by `memory`.
        memory: a list of optional tensors, optional
            a list of memory tensors as additional inputs for the attention to
            attend to. the size of the list is equal to the number of layers of
            SRUpp module. memory[i] is the memory tensor for the (i+1)-th layer
            and its second dimension (batch size) and third dimension (hidden
            size) must be compatible with the input tensor to the (i+1)-th layer.
        memory_mask_pad: tensor, optional
            the mask tensor indicate if a position in the memory tensors is an
            invalid / pad token that should be ignored in attention.
        ...
        memory_bank: Dict[str, List[Tensor]]
            a dictionary that stores various internal states indexed by state
            names. each value is a list of tensors in which the i-th element is
            the state tensor of the (i+1)-th layer. these internal states can
            be reused for attention for the next forward call during training
            and decoding.
        """
        ...
        if c0 is None:
            zeros = torch.zeros(bsz, output_size,
                                dtype=input.dtype, device=input.device)
            ...
            if list(c0.size()) != [num_layers, bsz, output_size]:
                raise ValueError("c0 has size {} but expect {}.".format(
                    list(c0.size()), [num_layers, bsz, output_size]
                ))
            c0_ = [x.squeeze(0) for x in c0.chunk(self.num_layers, 0)]

        if mask_pad is not None and list(mask_pad.size()) != [length, bsz]:
            raise ValueError("mask_pad has size {} but expect {}.".format(
                list(mask_pad.size()), [length, bsz]
            ))
        ...
        for rnn in self.rnn_lst:
            ...
            h, c = rnn(x, c0_[i], mask_pad=mask_pad, attn_mask=attn_mask,
                       memory=memory_i, memory_mask_pad=memory_mask_pad)
            x = h
            lstc.append(c)
            i += 1
        ...
        lstc_stack = torch.stack(lstc)
        if self.nn_rnn_compatible_return:
            lstc_stack = lstc_stack.view(num_layers, bsz,
                                         self.num_directions, self.hidden_size)
            lstc_stack = lstc_stack.transpose(1, 2).contiguous()
            lstc_stack = lstc_stack.view(num_layers * self.num_directions,
                                         bsz, self.hidden_size)

        if isinstance(orig_input, PackedSequence):
            h = nn.utils.rnn.pack_padded_sequence(h, lengths,
                                                  enforce_sorted=False)
